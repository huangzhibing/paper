...
<%@ attribute name="targetId" type="java.lang.String" required="false" description="额外要显示的文本框的id" %>
<%@ attribute name="targetFiled" type="java.lang.String" required="false" description="额外要显示的字段" %>
		...
		top.layer.open({
		    ...
		    btn: ['确定', '关闭']
    	       ,yes: function(index, layero){ //或者使用btn1
						...
						var ids = [], names = [], nodes = [] , codes =[] , targetids = [];
						if ("${checked}" == "true"){
						...
						for(var i=0; i<nodes.length; i++) {//<c:if test="${checked && notAllowSelectParent}">
							...
							ids.push(nodes[i].original.code);
							codes.push(nodes[i].original.name);
							names.push(nodes[i].text);//<c:if test="${!checked}">
                            targetids.push(nodes[i].original['${targetFiled}']);
                            //console.log("targetfiled:"+${targetFiled});
                            //console.log("targetids:"+nodes[i].original['${targetFiled}']);
							break; // 如果为非复选框选择，则返回第一个选择  </c:if>
						}
						$("#${id}Id").val(ids.join(",").replace(/u_/ig,""));
						$("#${id}Name").val(codes.join(","));
						$("#${targetId}").val(names.join(","));
						$("#${targetFiled}").val(targetids.join(","));
						$("#${id}Name").focus();
