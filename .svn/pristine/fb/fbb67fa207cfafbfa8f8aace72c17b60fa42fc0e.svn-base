...
$(document).ready(function() {
	$('#paperTable').bootstrapTable({
		  ...
		  //请求方法
               ...
               url: "${ctx}/paperReview/review/data",
                   ...
                   if($el.data("item") == "edit"){
                       ...
                       view(row.id);
                        ...
                        jp.confirm('确认要删除该评审记录记录吗？', function(){
                       	jp.loading();
                       	jp.get("${ctx}/paperReview/review/delete?id="+row.id, function(data){
                   	  		if(data.success){
			...
			/*,{
		        ...
		        ,formatter:function(value, row , index){
				   ...
				   <c:choose>
					   <c:when test="${fns:hasPermission('review:paper:edit')}">
					      return "<a href='javascript:edit(\""+row.id+"\")'>"+value+"</a>";
				      </c:when>
					  <c:when test="${fns:hasPermission('review:paper:view')}">
			...
			,{
		        field: 'student.specialityManage.zymc',
		        title: '专业名称',
		        sortable: true,
		        sortName: 'student.specialityManage.zymc',
  ...
  function deleteAll(){
		...
		jp.confirm('确认要删除该评审记录记录吗？', function(){
			jp.loading();  	
			jp.get("${ctx}/paperReview/review/deleteAll?ids=" + getIdSelections(), function(data){
         	  		if(data.success){
         	  			$('#paperTable').bootstrapTable('refresh');
         	  			jp.success(data.msg);
         	  		}else{
  ...
  function edit(id){
	  ...
	  jp.go("${ctx}/paperReview/review/form/edit?id=" + id);
  }
