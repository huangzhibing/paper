...
<script>
		...
		$(document).ready(function() {
			$crawTaskTypeTreeTable=$('#crawTaskTypeTreeTable').treeTable({  
		    	   theme:'vsStyle',	           
					...
					column:0,
					checkbox: false,
		            url:'${ctx}/info/crawTaskType/getChildren?parentId=',  
		...
		function del(con,id){  
			jp.confirm('确认要删除任务类型吗？', function(){
				jp.loading();
	       	  	$.get("${ctx}/info/crawTaskType/delete?id="+id, function(data){
	       	  		if(data.success){
...
<script type="text/html" id="crawTaskTypeTreeTableTpl">
			<td>
			<c:choose>
			      <c:when test="${fns:hasPermission('info:crawTaskType:edit')}">
				    <a  href="#" onclick="jp.openSaveDialog('编辑任务类型', '${ctx}/info/crawTaskType/form?id={{d.row.id}}','800px', '500px')">
							{{d.row.remarks === undefined ? "": d.row.remarks}}
					</a>
			      </c:when>
			      <c:when test="${fns:hasPermission('info:crawTaskType:view')}">
				    <a  href="#" onclick="jp.openViewDialog('查看任务类型', '${ctx}/info/crawTaskType/form?id={{d.row.id}}','800px', '500px')">
							{{d.row.remarks === undefined ? "": d.row.remarks}}
					</a>
			      </c:when>
			      <c:otherwise>
			...
			<td>
				<div class="btn-group">
				  ...
				  <ul class="dropdown-menu" role="menu">
					<shiro:hasPermission name="info:crawTaskType:view">
						<li><a href="#" onclick="jp.openViewDialog('查看任务类型', '${ctx}/info/crawTaskType/form?id={{d.row.id}}','800px', '500px')"><i class="fa fa-search-plus"></i> 查看</a></li>
					</shiro:hasPermission>
					<shiro:hasPermission name="info:crawTaskType:edit">
						<li><a href="#" onclick="jp.openSaveDialog('修改任务类型', '${ctx}/info/crawTaskType/form?id={{d.row.id}}','800px', '500px')"><i class="fa fa-edit"></i> 修改</a></li>
		   			</shiro:hasPermission>
		   			<shiro:hasPermission name="info:crawTaskType:del">
		   				<li><a  onclick="return del(this, '{{d.row.id}}')"><i class="fa fa-trash"></i> 删除</a></li>
					</shiro:hasPermission>
		   			<shiro:hasPermission name="info:crawTaskType:add">
						<li><a href="#" onclick="jp.openSaveDialog('添加下级任务类型', '${ctx}/info/crawTaskType/form?parent.id={{d.row.id}}','800px', '500px')"><i class="fa fa-plus"></i> 添加下级任务类型</a></li>
					</shiro:hasPermission>
				  </ul>
				</div>
			</td>
